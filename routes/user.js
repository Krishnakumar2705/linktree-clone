...
// @route PATCH /users/:username/bio
// @desc Update a user's bio
// @access Private
router.patch('/:username/bio', verify, (req, res) => {
  const username = req.params.username;
  const queryUsername = '^' + username + '$';
  User.findOne({ "username": { '$regex': queryUsername, $options: 'i' } })
    .select('-password -email')
...
// @route POST /users/:username/click/:linkId
  ...
  User.findOne({ "username": { '$regex': queryUsername, $options: 'i' } })
    .then(user => {
      ...
      if (link) {
        ...
        user.save()
          .then(() => res.json({ success: true, clicks: link.clicks }))
          .catch(err => res.status(400).json('Error: ' + err));
      } else {
...
// @route GET /users/:username/analytics
// @desc Get analytics for user's links
// @access Private
router.get('/:username/analytics', verify, (req, res) => {
  const username = req.params.username;
  const queryUsername = '^' + username + '$';
  ...
  User.findOne({ "username": { '$regex': queryUsername, $options: 'i' } })
    .select('-password -email')
    .then(user => {
      const analytics = user.links.map(link => ({
        _id: link._id,
        linkTitle: link.linkTitle,
        url: link.url,
        clicks: link.clicks || 0,
        recentClicks: (link.clickHistory || []).slice(-10)
      }));
      res.json(analytics);
    })
    .catch(err => res.status(400).json('Error: ' + err))
...
// @route PATCH /users/:username/reorder
// @desc Reorder links
// @access Private
router.patch('/:username/reorder', verify, (req, res) => {
  const username = req.params.username;
...
// @route PATCH /users/:username/emoji
// @desc Update a user's profile emoji
// @access Private
router.patch('/:username/emoji', verify, (req, res) => {
  const username = req.params.username;
  const queryUsername = '^' + username + '$';
  User.findOne({ "username": { '$regex': queryUsername, $options: 'i' } })
    .select('-password -email')
    .then(user => {
      user.profileEmoji = req.body.emoji;
      user.save()
        .then(user => res.json(user))
        .catch(err => res.status(400).json('Error: ' + err));
    })
